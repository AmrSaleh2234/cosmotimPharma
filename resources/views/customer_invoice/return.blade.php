...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"></h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0">/ ارجاع من الفاتورة</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
    ...
    <div class="row d-flex flex-wrap">
                            ...
                            <table class="table mg-b-0 text-md-nowrap">
                                ...
                                @foreach($invoice->inventory as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->pivot->price_after_discount/$item->pivot->quantity}}</td>
                                        <td>{{$item->pivot->quantity}}</td>
                                        <td>
                                            <a>
                                                <button class="btn btn-success btn-icon add-product-btn"
                                                        data-name="{{$item->product->name}}"
                                                        data-price="{{$item->product->price_after}}"
                                                        data-id="{{$item->pivot->id}}"
                                                        id="product-{{$item->product->id}}"><i
        ...
        <div class="col-lg-6 col-sm-12">
            <div class="card" style="border-top:3px solid cadetblue">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h3 class="card-title mg-b-0">المرتجع</h3>
                        <i class="mdi mdi-dots-horizontal text-gray"></i>
                    </div>
                    <form action="{{route('invoice_customer.doReturn',$invoice)}}" method="post">
                        @csrf
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table mg-b-0 text-md-nowrap">
                            ...
                            <div class="mt-4 d-flex justify-content-center">
                                ...
                                <input class="" type="number" value="0" min="0" max="100" disabled
                                       name="invoice_discount" id="discount_invoice">
                            </div>
                            <div class="mt-3">
                            ...
                            <div class="text-center">
                                <button type="submit" disabled class="btn btn-primary mt-3  enableByJs"
                                        id="order_customer" style="width: 32%">
                                    احفظ
                                </button>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function () {
            $('.add-product-btn').on('click', function (e) {
                ...
                var html = '<tr> <td>' + name + '</td><td class="quantity">' + '<input type="hidden" name="orders_id[]" value= "' + id + '"> ' + '<input class="input-sm quantity_input" value="1" data-price="' + price + '" type="number" min ="1"  name="quantities[]" data-price ="' + price + '"style="width:60px">' + '</td><td class="discount"><input disabled type ="number" min="0" max="100" style="width:49px" value ="{{$invoice->customer->discount}}" class="discount_input" name="discount[]" ></td><td class="product_price" >' + price + '</td><td class=""><input  type ="number" min="0"  style="width:49px" value=0   name="damage[]" ></td><td><button class="btn btn-danger btn-icon btn-delete-product" data-id = "' + id + '"><i class="typcn typcn-document-delete "></i></button></td> </tr>';
